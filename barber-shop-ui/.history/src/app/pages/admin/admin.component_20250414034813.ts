...
export class AdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.loadSchedules(); // inicializa com os dados da semana atual
  }
  ...
  loadSchedules(): void {
    this.scheduleService.getAll().subscribe({
      ...
      error: (err) =>
        console.error('Erro ao carregar agendamentos iniciais:', err),
    });
  }
  ...
  onFilterSubmit(): void {
    ...
    if (canceled !== null) params.canceled = canceled;
    if (haircutTypeId) params.haircutTypeId = haircutTypeId;
    if (!haircutTypeId) {
      this.snackBar.open('Selecione um tipo de corte.', 'Fechar', {
        duration: 3000,
      });
      return;
    }

    // Se os filtros confirmed e canceled não forem passados, retorna tudo
    if (
      params.confirmed === undefined &&
      params.canceled === undefined &&
      !params.haircutTypeId
    ) {
      this.scheduleService.getAll().subscribe({
