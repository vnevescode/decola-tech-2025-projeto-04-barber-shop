...
export class DashboardComponent implements OnInit {
  ...
  isFullyBooked(day: Date): boolean {
    ...
    return this.fullDays.some((d) => d.getTime() === target.getTime());
  }

  onDateSelected(date: Date) {
    this.selectedDate = date;
    this.availableTimes = this.generateTimeSlots();
    this.occupiedTimes = this.getOccupiedTimesForDay(date);
  }
  ...
  isTimeOccupied(time: string): boolean {
    return this.occupiedTimes.includes(time);
  }
  ...
  onSubmit() {
    ...
    const haircut = this.haircutTypes.find((h) => h.id === haircutTypeId);
    const haircutName = haircut ? haircut.name : 'Desconhecido';
    const haircutPrice = haircut ? haircut.price : '0.00';
      ...
      if (confirmed) {
        this.clientService.getCurrentClient().subscribe({
          next: (client) => {
            ...
            this.scheduleService.create(scheduleData).subscribe((created) => {
              this.snackBar.open(
                `✅ Agendamento para ${created.haircutName} confirmado!`,
                'Fechar',
                { duration: 3000 }
              );
