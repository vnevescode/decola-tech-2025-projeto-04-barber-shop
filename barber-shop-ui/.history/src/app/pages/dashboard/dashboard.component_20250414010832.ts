...
export class DashboardComponent implements OnInit {
  ...
  isTimeSlotTaken(time: string): boolean {
    ...
    return this.allSchedules.some((sch) => {
      const schDate = sch.startAt.split('T')[0];
      const schTime = sch.startAt.split('T')[1].substring(0, 5);
      return schDate === dateStr && schTime === time;
    });
  }

  highlightFullyBookedDay = (day: any): void => {
    const dateStr = day.date.toISOString().split('T')[0];
    if (this.fullyBookedDates.has(dateStr)) {
      day.cssClass = 'fully-booked';
    }
  };
  ...
  loadAllSchedules(): void {
    this.scheduleService.getAllSchedules().subscribe((schedules) => {
      ...
      this.calculateFullyBookedDays();
    });
  }

  calculateFullyBookedDays(): void {
    const grouped: { [key: string]: number } = {};
    for (const schedule of this.allSchedules) {
      const dateStr = schedule.startAt.split('T')[0];
      grouped[dateStr] = (grouped[dateStr] || 0) + 1;
    }
    const totalSlotsPerDay = this.generateTimeSlots().length;
    this.fullyBookedDates = new Set(
      Object.entries(grouped)
        .filter(([_, count]) => count >= totalSlotsPerDay)
        .map(([date]) => date)
    );
  }
