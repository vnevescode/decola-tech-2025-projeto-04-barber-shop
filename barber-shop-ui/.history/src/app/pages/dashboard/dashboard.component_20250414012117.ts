...
export class DashboardComponent implements OnInit {
  ...
  generateTimeSlots(date: Date): string[] {
    ...
    const scheduledForDay = this.monthlySchedules.filter(
      (s) => new Date(s.startAt).toDateString() === date.toDateString()
    );
    const occupied = scheduledForDay.map((s) =>
      new Date(s.startAt).toTimeString().substring(0, 5)
    );
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      ...
      slots.forEach((time) => {
        if (!occupied.includes(time)) times.push(time);
      });
    }
  ...
  cellModifier = (day: any) => {
    const isFull = this.isFullyBooked(day.date);
    return {
      cssClass: isFull ? 'fully-booked-day' : '',
    };
  };
